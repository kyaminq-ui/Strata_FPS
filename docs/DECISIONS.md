...
47. **Boss = `Enemy` + `BossPhases`, phases = deux `EnemyConfig`** : pas de nouvelle IA ; la phase 2 change les valeurs (vitesse, tir à plombs, réaction), ajoute 2 agents (spawner manuel) et une onde de choc télégraphiée qui force à bouger ou sauter. Le host décide, tous appliquent la config pour l'affichage. Un reset de rencontre remet la phase 1. Barre de vie, animations, sons et un troisième pattern viendront avec l'art/le polish si le combat en manque.
46. **Boucle de mission = changement d'arène par RPC du host** (pas de `change_scene`) : `main.gd` garde son rôle de chargeur, `GameSession` n'ose qu'un signal (`arena_requested`). Le hub est un intermède sans ennemi ; l'ascenseur réutilise `HackTerminal` (maintenir F) pour rester dans le schéma client → host validé. Fin de mission = tous les objectifs faits, écran de fin puis retour automatique au hub (12 s). Aucune progression n'est conservée pour l'instant (upgrades = plus tard). Les sondes partent de `--arena=` explicite ; l'arène par défaut du menu reste l'arène de test.
45. **Missions = objectifs dans `GameSession` + composants d'arène** : le host décide et diffuse ; `HackTerminal` (jauge host-autoritaire, même schéma client → host que la réanimation), `MissionDoor` (verrou), `SectorMission` (câblage propre au secteur : hack puis cible). Deux « missions » = deux objectifs du même contrat pour la vertical slice. Le terminal est dans un intérieur (entrepôt ouest) pour offrir une vraie voie discrète ; les portes du boss forcent l'ordre sans imposer le style. Aucune énigme à deux joueurs. Le boss (4.5) remplace l'élite comme cible.
44. **Blockout généré par script** : le secteur et le hub sont produits par `tools/build_bas_fonds.py` (listes de boîtes et de rampes, couleurs par zone) plutôt qu'édités à la main dans Godot : itération rapide, positions relisibles, cohérence avec les métriques. Les .tscn générés sont commités ; les modifier à la main serait écrasé au prochain lancement. Une rampe qui rejoint un niveau doit dépasser de 0.1 m (sinon arête → mur). Le passage hub → secteur se fera avec les missions (4.4).
43. **Métriques de niveau verrouillées** : les valeurs de `default_movement.tres` sont figées (feel validé) ; les règles de blockout (passages 1.2/2.5 m, étage 3.5 m, vides 4.5/6/10 m, mur de wall-run ≥ 4 m, passage bas 1.3 m, pas de marches) viennent de mesures dans le jeu (`MOVEMENT_METRICS.md`) avec marge. Pas d'escalier : rampes ≤ 35° ou blocs sautables.
42. **`GameSession` = autoload mince + `Checkpoint` = Area3D** : le host retient le dernier checkpoint touché ; le respawn (mort solo, timeout de down, tous down) se fait dessus, sinon sur les marqueurs de départ. **Reset de rencontre** = quand plus personne n'est debout : tous les ennemis reviennent à leur poste, renforts supprimés (choix le plus simple, comportement type « checkpoint de jeu solo » ; cadavres compris). Pas de sauvegarde. Piège : les sondes headless `-s` compilent avant l'enregistrement des autoloads → elles ne doivent référencer aucune classe du jeu (`Player`, `Enemy`, `WeaponController`...), seulement des nœuds non typés.
40. **Lag compensation analytique, fenêtre 0.3 s** : le rembobinage physique (déplacer le corps puis raycast) est sans effet avec Jolt (déplacement appliqué au pas suivant), donc le host teste les ennemis par rayon-capsule à leur position passée. `window` 0.3 s (couvre ~RTT 200 ms), valeur unique dans `LagCompensator` (l'override 0.15 de `enemy.tscn` est supprimé). Fenêtre fixe, pas adaptative : suffisant pour un coop PvE entre amis ; à revoir seulement si des joueurs à >250 ms de RTT se plaignent.
41. **Latence simulée dans le code** : `DelayedPeer` enveloppe le peer ENet (dans `MultiplayerManager`, seul point qui connaît le transport), activé par `--lag`/`--jitter`. Outil de test, sans effet quand `lag_ms` = 0.
39. **Lean = composant `PlayerLean`, cible répliquée `net_lean`** (-1..1, propriétaire, non fiable) : l'autorité limite contre les murs, chaque peer lisse et applique décalage/roulis de la tête et du mesh. Pas de déplacement de la hitbox (simplicité). `interact` (réanimer) déplacé de E à F pour libérer E (lean droite). Mapping souris : Mouse5 (bouton latéral 2) = gauche, Mouse4 (bouton latéral 1) = droite.
