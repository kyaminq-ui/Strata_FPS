...
`GameSession` (autoload) garde la liste d'objectifs, écrite par le host (`set_objectives`, `complete_objective`) et diffusée par un RPC fiable `_sync_objectives` (`authority`, `call_local`) ; un pair qui se connecte reçoit l'état courant (`peer_connected`). `HackTerminal` : le joueur local envoie `request_hack(active)` (RPC vers le host), le host valide (joueur debout, `net_position` à portée ×1.5) et fait progresser la jauge ; il diffuse `sync_state(progress, done)` (non fiable, 10 Hz). `MissionDoor.open()` (host) → RPC fiable `set_open` chez tous. Les nœuds de mission sont statiques dans l'arène (mêmes chemins). Sonde : `tests/net_probe_hack.gd`.

## Boss
`BossPhases` (enfant de l'Enemy boss, mêmes chemins chez tous) : le host surveille les PV, décide la phase 2 et diffuse `_set_phase` (RPC fiable, `call_local`) ; chaque pair applique `Enemy.apply_config` (nom, couleur ; les valeurs de combat ne servent qu'au host). Renforts : `BossAdds` (`EnemySpawner`, spawn_function, préfixe et groupe propres pour ne pas heurter les renforts d'alerte). Onde de choc : le host diffuse `_show_telegraph` (disque cosmétique) puis applique les dégâts aux joueurs debout dans le rayon et sous `shockwave_dodge_height`. `GameSession.announce(text)` : message court chez tous. Sonde : `tests/net_probe_boss.gd`.

## Changement d'arène (hub <-> secteur)
Le host appelle `GameSession.load_arena(index)` : RPC fiable `authority` + `call_local` → signal `arena_requested` → `main.gd` remplace l'arène (`remove_child` immédiat, nouveau nœud du même nom possible). Ordre critique : chez le **host**, le chargement est différé (l'ordre part avant ses spawns) ; chez un **client**, il est immédiat dans le handler du RPC (sinon les spawns du host, arrivés dans le même paquet, trouveraient l'arène absente). `PlayerSpawner._ready` spawne le host et tous les pairs déjà connectés. `GameSession.reset()` à chaque chargement ; les objectifs sont renvoyés par le host quand l'arène est prête. Erreurs bénignes attendues : `ERR_UNAUTHORIZED` (`!pinfo.recv_nodes.has(net_id)`) et « Node not found … /Sync » pour des paquets non fiables en vol vers l'ancienne arène. Sonde : `tests/net_probe_loop.gd`.
