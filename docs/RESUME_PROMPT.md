...
ÉTAT EN UNE PHRASE
Milestones 0, 1 et 2 sont terminés et validés par le développeur : mouvement complet (marche, saut, dash, slide, wall-run), 2 joueurs en réseau, 2 armes hitscan, mêlée, grenade explosive répliquée, santé/down/réanimation. Il n'y a encore AUCUNE IA, mission, secteur, boss, audio ni asset final (graybox uniquement).

MISSION DE CETTE SESSION
Milestone 3 — IA & infiltration, host-autoritaire, en commençant par la tranche 3.1 (ennemi de base + navigation + patrouille, réplication réseau, blessable par les armes existantes) telle que décrite dans docs/NEXTSTEPS.md §7. Avance tranche par tranche (3.1 → 3.6), chacune vérifiée en solo ET en host+client avant la suivante.
...
- Hors scope (ne pas développer) : PvP, split-screen, matchmaking public, Steam, boss, arbre de compétences, sauvegarde complète, menus définitifs, graphismes/shaders définitifs, génération Meshy massive, audio final.
...
3. Fais-moi un résumé de 10 lignes de ce que tu as compris et propose le plan précis de la tranche 3.1.
...
Pendant le travail : explique brièvement ce que tu vas faire, implémente le plus petit changement viable, teste, corrige, mets à jour docs/TASKS.md, NETWORK.md et DECISIONS.md, puis propose un message de commit à la fin de chaque tranche.
```
